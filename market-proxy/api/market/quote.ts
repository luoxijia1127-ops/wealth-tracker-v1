 ...
 * GET /api/market/quote?symbol=AAPL&mic=XNAS&asOf=2026-04-15  （取 <= asOf 的最近日 K 收盘）
 *
 * 统一返回：{ ok, close, tradeDate, currency, symbol, mic_code, source }
 */
...
export default async function handler(
  ...
  try {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(asOf)) {
      ...
      if (!r.ok) {
        res.status(502).json({ error: 'Upstream quote error', status: r.status });
        return;
      }
      ...
      const dt =
        typeof json.datetime === 'string' && /^\d{4}-\d{2}-\d{2}/.test(json.datetime)
    ...
    const text = await r.text();
    if (!r.ok) {
      res.status(502).json({ error: 'Upstream time_series error', status: r.status });
      return;
    }
